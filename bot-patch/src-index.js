...
// ---- CORS: allows the separate Crevio Web App (different Render domain) to call /api/v1 ----
// Only added in front of /api/v1 — the bot webhook itself doesn't need it.
app.use('/api/v1', (req, res, next) => {
  const allowedOrigin = process.env.WEB_APP_ORIGIN || '*'; // set WEB_APP_ORIGIN in Render env vars for tighter security
...
app.post('/webhook/razorpay/:creatorId', async (req, res) => {
  try {
    res.status(200).send('OK');
    handleRazorpayWebhook(req).catch(err => console.error('Razorpay webhook error:', err.message));
  } catch (err) {
